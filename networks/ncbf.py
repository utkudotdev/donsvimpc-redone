...
from .feature import make_dubins_features
import optax
...
class NCBF(eqx.Module):
    ...
    def __call__(self, state: State, params: Parameters):
        relative_state = make_dubins_features(state, params)
        V = self.ncbf_network(relative_state)
        h = self.h_fn(state, params)
        return jnp.max(jnp.array([h, V]), axis=0)


def compute_ncbf_target(
    ncbf: NCBFNetwork, gamma: float, h_t: jnp.ndarray, x_t1: jnp.ndarray
):
...
def load_checkpoint(
    ...
    with open(state_path, "rb") as f:
        model, opt_state, epoch = eqx.tree_deserialise_leaves(
            f, (model_template, opt_state_template, 0)
        )

    return model, optimizer, opt_state, optimizer_params, epoch
