...
def cbf_violation(h_fn, dt):
    def compute_cbf_violation(s: State, a: jnp.ndarray, p: Parameters, alpha: float):
        h = jnp.max(h_fn(s, p))
...
def embed_cbf_violation(
    h_vio_fn, cost_fn, terminal_cost_fn, alpha: float, vio_cost: float
):
